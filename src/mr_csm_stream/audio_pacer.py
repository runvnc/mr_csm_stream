...
class AudioPacer:
    ...
    def __init__(self, sample_rate: int = 8000):
        ...
        self._chunks_sent = 0
        self._first_chunk_recv_time: Optional[float] = None
        self._first_chunk_send_time: Optional[float] = None
    ...
    async def add_chunk(self, audio_bytes: bytes):
        ...
        if self._running:
            ...
            if self._first_chunk_recv_time is None:
                self._first_chunk_recv_time = recv_time
                logger.info(f"PACER_RECV: First chunk received at {recv_time:.3f}")
            
            logger.info(
                f"PACER_RECV: chunk #{self._chunks_received}, {len(audio_bytes)} bytes, "
    ...
    async def clear(self):
        ...
        self._chunks_received = 0
        self._chunks_sent = 0
        self._first_chunk_recv_time = None
        self._first_chunk_send_time = None
        self._interrupted = False
        self._finished_adding = False
    ...
    async def _pace_loop(self):
        ...
        while self._running:
            if len(self.buffer) > 0:
                ...
                if self._first_chunk_send_time is None:
                    self._first_chunk_send_time = send_start
                    latency = (send_start - self._first_chunk_recv_time) * 1000 if self._first_chunk_recv_time else 0
                    logger.info(f"PACER_SEND: First chunk sending at {send_start:.3f}, latency from recv={latency:.1f}ms")
                
                logger.info(
                    f"PACER_SEND: chunk #{self._chunks_sent}, {len(chunk)} bytes, buffer_remaining={len(self.buffer)}"
                )
                ...
                try:
                    ...
                    if result is False:
                        ...
                        logger.info(f"PACER_SEND: callback returned False, took {(send_end-send_start)*1000:.1f}ms")
                ...
                send_end = time.perf_counter()
                logger.debug(f"PACER_SEND: callback took {(send_end-send_start)*1000:.1f}ms")
                ...
                if sleep_duration > 0:
                    ...
                    logger.debug(f"PACER_SEND: slept {sleep_duration*1000:.1f}ms for pacing")
